...
### S. Google Nest Mini (2nd gen) / H2C

- on an owned/authorized exact H2C unit, preserve firmware/software state, account/Home assignment and network state before testing;
- reproduce one harmless Local Home SDK integration through the documented developer path and preserve a local experiment receipt without promoting the sandbox to shell/root authority;
- record the Local Home app's demand-loaded lifecycle and observe whether memory pressure/idle unload behavior materially affects the intended workload rather than assuming persistent service semantics;
- test LAN fulfillment with WAN deliberately unavailable only after normal provisioning, while keeping local device control separate from the Cloud-to-cloud SYNC/account/fallback dependencies documented by Google;
- test normal restart and cold power loss -> device recovery -> Local Home intent lifecycle rather than assuming custom code autostarts as a daemon;
- measure wall power under defined microphone/network/runtime profiles; keep Google's 1.6 W EN 50564 networked-standby result as manufacturer standardized evidence, not a substitute for local workload measurement;
- test factory reset only on deliberately noncritical state and preserve device-local data impact separately from Google Home / developer-cloud account state;
